...
(you must specify the exact number of values in the interface). Each of the float values is given its own continuous channel, with the float value combined with the provided timestamp
(or a timestamp generated by open ephys if none is given). You can view the raw values in openephys using the LFP viewer, though it's not going to be that interesting in that form.

The intention is that the data is coming from Bonsai, with the float values corresponding to some kind of x and y values. 

In order to get acurate timestamps you probably want to have a ttl output from your camera directly into the main acquistion board, and then have a customer recorder plugin that can
